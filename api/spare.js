...
function conf(){return {version:'3.1-public-demo',speech:'OpenAI Realtime',model:process.env.OPENAI_REALTIME_MODEL||'gpt-realtime',voice:process.env.OPENAI_REALTIME_VOICE||'marin',designer:process.env.GENERAL_COMPUTE_API_KEY?'General Compute':'OpenAI (no sponsor inference)',access_required:false,missing:['OPENAI_API_KEY'].filter(k=>!process.env[k]),connection_minutes:Math.max(3,Math.min(65,Number(process.env.MAX_CONNECTION_MINUTES)||15))};}
